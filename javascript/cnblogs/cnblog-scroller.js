jQuery(function($) {
	...
	function loadScroller() {
		...
		// 绝对定位距离顶部高度
        var topValue = $("#header").height() + 20;
        // top_nav的高度
        var topNavHeight = $("#top_nav").height();
		// 正则表达式，配置数组
		var hxReg = RegExp(/\d/);
		if($ph.length > 0 || $postTitle.length > 0) {
			...
			//取当前边栏的宽度
			$('#sidebar_scroller').css({'width': $('#blog-sidecolumn').width(), 'top': (topNavHeight + topValue) + 'px', 'max-height': window.screen.availHeight - topValue - 46});
			//让导航目录停留在页面顶端
			...
			$(document).on('scroll', debounce(function() {
				if($(document).scrollTop() == 0) {
					$("#sidebar_scroller").animate({top: (topNavHeight + topValue) + 'px', position: 'fixed;!important'}, 300);
				}
			}, 800))
		}
	}
